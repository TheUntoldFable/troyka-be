...
const stripe = require('stripe')(process.env.NODE_ENV === 'development'
...
module.exports = createCoreController('api::order.order', ({ strapi }) => ({
    async create(ctx) {
        const { products, paymentMethod, status, addressInfo, credentialsInfo, billingAddressInfo, user, totalPrice, } = ctx.request.body;
        const origin = ctx.get('origin') || ctx.request.header.origin;
        const resolveProductLocale = (product) => product?.locale ?? product?.attributes?.locale;
        const fetchProduct = async (product) => {
            if (!product?.id)
                throw new Error('Missing product id in order payload');
            const locale = resolveProductLocale(product);
            const queryOptions = { fields: ['name', 'price'] };
            if (locale)
                queryOptions.locale = locale;
            const item = await strapi.entityService.findOne('api::product.product', product.id, queryOptions);
            if (!item) {
                throw new Error(`Product ${product.id} not found${locale ? ` for locale ${locale}` : ''}`);
            }
            return item;
        };
        try {
            if (paymentMethod === 'card') {
                const orderId = await (0, uuid_1.v4)();
